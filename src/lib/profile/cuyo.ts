...
export function positionAt(alongKm: number): ProfilePoint & { azimuth: number } {
  const result = line.Position(alongKm * 1000);
...
export function corridorEdge(alongKm: number, side: -1 | 1): ProfilePoint {
  const center = positionAt(alongKm);
  const result = geod.Direct(center.latitude, center.longitude, center.azimuth + side * 90, CUYO.halfWidthKm * 1000);
...
export function projectToCuyo(longitude: number, latitude: number): { alongKm: number; crossKm: number } {
  const fromA = geod.Inverse(CUYO.a.latitude, CUYO.a.longitude, latitude, longitude);
  const heading = ((fromA.azi1 ?? 0) - initialAzimuth) * Math.PI / 180;
  const guess = (fromA.s12 ?? 0) * Math.cos(heading) / 1000;
  let left = Math.max(-150, guess - 85);
  let right = Math.min(CUYO_LENGTH_KM + 150, guess + 85);
  const ratio = (Math.sqrt(5) - 1) / 2;
  const distance = (s: number) => {
    const foot = positionAt(s);
...
export function projectEvent(feature: InpresFeature): ProjectedEvent | null {
  ...
  if (longitude < -75 || longitude > -59 || latitude < -33 || latitude > -29) return null;
  const projected = projectToCuyo(longitude, latitude);
  if (projected.alongKm < 0 || projected.alongKm > CUYO_LENGTH_KM || projected.crossKm > CUYO.halfWidthKm) return null;
...
export function selectCuyoEvents(features: InpresFeature[]): ProjectedEvent[] {
  ...
  for (const feature of features) {
    const point = projectEvent(feature);
...
export function sampleCuyoProfile(gebco: GebcoArtifact, slab: Slab2Artifact): ProfileSample[] {
  ...
  return Array.from({ length: steps + 1 }, (_, index) => {
    const alongKm = CUYO_LENGTH_KM * index / steps;
    const point = positionAt(alongKm);
    return {
      alongKm, point,
